...
- 恢复标准的 FoloToy AI Passport 主菜单，并将接鸡蛋游戏作为普通菜单项；在游戏中长按 `OK` 会返回系统菜单，而不是进入仅属于游戏的标题页。
- 新增透明的左右镜像手部/篮筐前壁图层和固定 180 毫秒接住动画：鸡蛋先完整显示 90 毫秒，再在篮筐前壁后最多下沉 4 像素 90 毫秒，随后消失，不再出现篮筐下方的画面。
- 增强每 15 个鸡蛋的速度提升幅度，同时保持上下轨道共用同一移动速度，并在每个提速门槛显示 `SPEED UP!` 提示。
